...
func testUserToOneTenantUsingTenant(t *testing.T) {
	...
	slice := UserSlice{&local}
	if err = local.L.LoadTenant(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testUserToOneRemoveOpTenantUsingTenant(t *testing.T) {
	var err error
...
var (
	userDBTypes = map[string]string{`ID`: `int`, `FirstName`: `varchar`, `LastName`: `varchar`, `Email`: `varchar`, `Password`: `char`, `Active`: `tinyint`, `AccountID`: `int`, `TenantID`: `int`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`}
	_           = bytes.MinRead
)
