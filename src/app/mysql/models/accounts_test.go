...
func testAccountToOneTenantUsingTenant(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadTenant(ctx, tx, false, (*[]*Account)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testAccountToOneSetOpTenantUsingTenant(t *testing.T) {
	var err error
...
func testAccountToOneRemoveOpTenantUsingTenant(t *testing.T) {
	var err error
...
var (
	accountDBTypes = map[string]string{`ID`: `int`, `Name`: `varchar`, `TenantID`: `int`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`}
	_              = bytes.MinRead
)
