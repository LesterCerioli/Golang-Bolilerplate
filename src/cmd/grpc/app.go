...
func main() {
	...
	*/
	...
	// Business Service
	businessService := service.NewBusinessService(db, validator, passwordHandler, logger)
	grpcBusinessServer := grpcTransport.BusinessServer{BusinessService: businessService, Logger: logger}
	pb.RegisterBusinessServer(grpcServer, &grpcBusinessServer)
	app.RegisterBusinessServiceValidators(businessService, validator, logger) // Add Custom validators
	...
	err = pb.RegisterBusinessHandlerFromEndpoint(ctx, mux, ":"+conf.GetString("app.grpc.port"), opts)
	if err != nil {
		logger.Sugar().Errorf("Failed to register Visit Service %+v", err)
	}
